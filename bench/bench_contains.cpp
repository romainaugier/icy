                   ...
                   const icy::set<Key, N>& icy_set)
{
    ...
    std::vector<Result> map_results = {
        ...
        { "sorted vector+bs", measure(body_map_sorted, iters) / ops_per_body },
    };

    print_section("map contains()", map_results);
    ...
    print_section("set contains()", set_results);

    // --- iteration (unchanged: contains() has no bearing on it) ---
...
int main(void)
{
    std::println("icy benchmark (contains variant, cycle source: {}, num trials: {})", bench::cycle_source(), NUM_TRIALS);
    std::println("");
